...
- **Arabic and English support is a hard requirement** for every component that touches text.
...
| OCR (scans, Arabic) | Azure AI Document Intelligence (UAE North) | Same, or Tesseract / PaddleOCR self-hosted |
...
| Access routes | Anthropic API direct (pilot); **Amazon Bedrock** from the AWS Middle East regions (UAE me-central-1, Bahrain me-south-1) using global cross-region inference; **Microsoft Foundry** (billed through the Microsoft marketplace). Route is a gateway setting. |
| Why | Strong performance on long, structured documents and on instruction-following for citations and refusals; enterprise terms; multiple GCC-reachable access routes. |
| Alternatives | OpenAI GPT-5 family via Azure OpenAI (also reachable in UAE North); Google Gemini via Vertex AI. All are supported by the gateway; a switch is configuration. |
...
| Alternatives | Cohere Embed v4 and Rerank 3.5 (multilingual, strong Arabic); OpenAI text-embedding-3-large; self-hosted **BGE-M3** and **bge-reranker-v2-m3** (open source, multilingual, no data leaves the environment). |
...
| Azure AI Document Intelligence | Cloud OCR and layout analysis with Arabic support, available in Azure UAE North for in-region processing. Priced per page (roughly USD 1.50 per 1,000 pages for read/layout tiers; check current list). Used only for scanned pages. |
| Alternatives | Google Document AI; AWS Textract (limited Arabic); self-hosted Tesseract or PaddleOCR (free, lower accuracy on poor scans); LlamaParse or Unstructured (managed parsing APIs). |

## 3.5 Database: PostgreSQL with pgvector

One database for records, full-text search and vectors. pgvector provides approximate nearest-neighbour search (HNSW) sufficient to millions of chunks; PostgreSQL full-text search handles exact numbers, names and Arabic terms; row-level security enforces per-user access inside the database. Supabase provides a managed PostgreSQL with storage and auth for the pilot (**Pro plan USD 25 per month plus usage**); enterprise tier uses the client's cloud provider's managed PostgreSQL in region. If the corpus grows past what pgvector handles comfortably, Qdrant or OpenSearch can be added behind the same retrieval interface.
...
Single sign-on through the client's identity provider (Microsoft Entra ID is the norm in the GCC; Okta, Google Workspace also supported) using OpenID Connect. Multi-factor authentication is enforced by the provider. Roles and per-document access are managed in the application. Keycloak is the self-hosted alternative for the sovereign pattern.
...
| WhatsApp | WhatsApp Business Platform (Meta Cloud API) with approved message templates; per-conversation pricing; strongly used in GCC relationship management |
