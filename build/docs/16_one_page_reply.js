...
We would put both on **one shared platform** so each is usable alone but they share a single login, one list of organisations and people, one document store and one audit trail. The investor named in a contract is then the same record as the investor in your pipeline, and you never maintain two contact lists. Everything runs in your own cloud account, in Malaysia (AWS and Microsoft both have regions there) or Singapore, since the Philippines has none yet. A gateway routes sensitive documents to a model in a country you name, so residency stays a setting you control. You own the code, the accounts and the data throughout.
...
| Answer in plain language | An independent **grounding check** reads the drafted answer back against its sources and deletes any sentence they do not support. If nothing survives, it declines. It works in the languages your documents use, including scans. |
| Contact and relationship records | A shared entity registry, so anything found in the documents can surface on the matching relationship. Pipelines and stages are configurable per category, including a GCC path. |
...
1. **A handful of sample documents.** Fifty to two hundred under NDA, mixed formats and languages, ideally your worst scans rather than your tidiest files. This is the most useful single input, because it sets a realistic accuracy baseline instead of an optimistic one.
...
5. **How you work today.** The pipeline stages your relationship managers actually use, who should approve outreach per category, how a GCC relationship differs from one at home, and any existing CRM we should work alongside.
